...
const byKey = (k: string): string => {
  ...
  if (matches.length === 0) {
    throw new Error(
      `[semaforo-de-impulsos/assets] Asset key not found in primaria manifest: "${k}". ` +
  ...
  if (!chosen) {
    throw new Error(`[semaforo-de-impulsos/assets] Empty match for "${k}"`);
  }
  return chosen.path;
};

export const ASSETS = {
  // Mascota TDAH primaria: "Mono Mensajero". El tema es autocontrol — un mono
  // travieso que entrega cartas SOLO cuando el semáforo le da paso.
  mascot: byKey("sprites_animal-pack_PNG_Round (outline)_monkey"),
  sounds: {
