...
2. Archivos: manifest.ts, Component.tsx, scene.ts, config.ts, rubric.ts
3. Registrar el manifest en packages/games-primaria/src/index.ts
4. La rúbrica debe convertir crudo → 0-3 según la fórmula z-score de CLAUDE.md
5. Telemetría completa al endpoint POST /api/challenge/result
6. Pantalla de resultado al final con: Likert resultante, métricas, feedback ¿fácil/difícil? ¿gustó?
